...
function HUDAssaultCorner:_animate_text(text_panel, bg_box, color, color_function)
	...
	while true do
		...
		if speed > 90 then speed = speed - 2 * self._scale end
...
function HUDAssaultCorner:_get_assault_strings()
	...
	if self._assault_mode == "endless" then
		return {
...
function HUDAssaultCorner:_start_assault(text_list)
	...
	if assaultbox_panel:child("text_panel") then
		...
		assaultbox_panel:child("text_panel"):set_w(HeistHUD.options.show_badge and assaultbox_panel:w() or assaultbox_panel:w() - 30 * self._scale)
	else
...
function HUDAssaultCorner:_animate_icon(icon_assaultbox)
	...
	while true do
		...
		for i = 1, difficulty do
			...
			skull:set_visible(icon_assaultbox:visible())
			if position then
